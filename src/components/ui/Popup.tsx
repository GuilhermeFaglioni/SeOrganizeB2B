...
interface PopupTriggerElementProps {
  id?: string;
  disabled?: boolean;
...
export function Popup(rawProps: PopupProps) {
  ...
  const triggerNode = triggerAsChild && isValidElement(trigger) ? (() => {
    const triggerElement = trigger as ReactElement<PopupTriggerElementProps>;
    const childOnClick = triggerElement.props.onClick as
