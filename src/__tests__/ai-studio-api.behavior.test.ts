...
import {
  GET as refineGET,
  POST as refinePOST,
} from "../app/api/ai/studio/refine/[templateId]/route";
import {
  DELETE as imagesDELETE,
  GET as imagesGET,
  POST as imagesPOST,
} from "../app/api/ai/studio/images/route";
...
describe("AI Studio API", () => {
  beforeEach(() => {
    ...
    mocks.getAIStudioConfig.mockResolvedValue({
      enabled: true,
      connections: [],
      consents: {},
    });
    mocks.getAIStudioMaxRequestBytes.mockReturnValue(240_000);
    mocks.recordAIStudioConsent.mockResolvedValue({
      provider: "openai",
      version: "v1",
    });
    mocks.generateTemplateCandidate.mockResolvedValue({
      requestId: "req-1",
      candidate: { html: "<p>ok</p>" },
    });
    mocks.renderAIStudioSyntheticPreview.mockReturnValue({
      html: "<p>Cliente Exemplo</p>",
      warnings: [],
    });
  });
  ...
  it("requires both template management and AI generation permissions", async () => {
    ...
    expect(mocks.denyFor).toHaveBeenNthCalledWith(
      2,
      "user-1",
  ...
  it("records explicit versioned consent before generation", async () => {
    const response = await consentPOST(
      request("http://x/api/ai/studio/consent", {
        accepted: true,
        provider: "openai",
        version: "v1",
      }),
    ...
    expect(mocks.recordAIStudioConsent).toHaveBeenCalledWith({
      tenantId: "tenant-1",
      actorId: "user-1",
      provider: "openai",
      version: "v1",
  ...
  it("passes the tenant and actor boundary into generation", async () => {
    const response = await generatePOST(
      request("http://x/api/ai/studio/generate", {
        provider: "openai",
        model: "gpt-4o",
        message: "Briefing",
        consentVersion: "v1",
  ...
  it("passes multipart image bytes directly into generation", async () => {
    ...
    const response = await generatePOST(
      new NextRequest("http://x/api/ai/studio/generate", {
        method: "POST",
        body: form,
      }),
    );
    ...
    expect(
      mocks.generateTemplateCandidate.mock.calls[0][0].imageFiles[0].data,
  ...
  it("passes the ephemeral context snapshot to generation without adding a history endpoint", async () => {
    const ephemeral = {
      ...
      recentMessages: [
        { role: "assistant", content: "A paleta foi definida." },
      ],
      sessionSummary: {
        focus: "Paleta azul",
        decisions: ["Usar paleta azul"],
        pending: ["Revisar rodapé"],
        variables: [],
      },
  ...
  it("renders preview from synthetic data through the server seam", async () => {
    const response = await previewPOST(
      request("http://x/api/ai/studio/preview", {
        html: "<p>{{cliente.nome}}</p>",
      }),
    );
    expect(response?.status).toBe(200);
    expect(mocks.renderAIStudioSyntheticPreview).toHaveBeenCalledWith(
      "<p>{{cliente.nome}}</p>",
      "pt-BR",
    );
  });
  ...
  it("loads the refinement base for the current tenant only", async () => {
    mocks.getAIStudioRefinementBase.mockResolvedValue({
      id: "template-1",
      name: "Base",
      html: "<p>ok</p>",
      warnings: [],
    ...
    const response = await refineGET(
      new NextRequest("http://x/api/ai/studio/refine/template-1"),
      { params: Promise.resolve({ templateId: "template-1" }) },
    );
    expect(response?.status).toBe(200);
    expect(mocks.getAIStudioRefinementBase).toHaveBeenCalledWith(
      "tenant-1",
      "template-1",
    );
  });
  ...
  it("denies the refinement base without AI permissions", async () => {
    mocks.denyFor.mockResolvedValueOnce(
      NextResponse.json(
        { data: null, error: { code: "FORBIDDEN" } },
        { status: 403 },
      ),
  ...
  it("updates the original only with explicit confirmation", async () => {
    mocks.updateRefinedTemplate.mockResolvedValue({
      template: { id: "template-1", name: "Base", html: "<p>novo</p>" },
      warnings: [],
      draftCount: 3,
    });
    const response = await refinePOST(
      request("http://x/api/ai/studio/refine/template-1", {
        html: "<p>novo</p>",
        confirmed: true,
      }),
      { params: Promise.resolve({ templateId: "template-1" }) },
  ...
  it("maps a missing update confirmation to a precondition response", async () => {
    mocks.updateRefinedTemplate.mockRejectedValue(
      new AIStudioError(
  ...
  it("maps a missing refinement template to 404", async () => {
    mocks.getAIStudioRefinementBase.mockRejectedValue(
      new AIStudioError("TEMPLATE_NOT_FOUND", "Não encontrado."),
    );
    const response = await refineGET(
      new NextRequest("http://x/api/ai/studio/refine/missing"),
  ...
  it("maps a payload-limited continuing session to 413", async () => {
    mocks.generateTemplateCandidate.mockRejectedValue(
      new AIStudioError("PAYLOAD_LIMITED", "Contexto excedeu o limite."),
    );
    const response = await generatePOST(
      request("http://x/api/ai/studio/generate", {
        provider: "openai",
  ...
  it.each(["CONSENT_REQUIRED", "RATE_LIMITED", "PAYLOAD_LIMITED"] as const)(
    "emits a streaming error event for %s without waiting for the provider",
    async (code) => {
      mocks.generateTemplateCandidate.mockRejectedValue(
        new AIStudioError(code, "Generation blocked."),
      );
  ...
  it("keeps a started stream at 200 and emits safe provider diagnostics after a delta", async () => {
    mocks.generateTemplateCandidate.mockImplementation(
      async (_input, hooks) => {
  ...
  it("passes a multipart image upload to the attach seam within the tenant and actor boundary", async () => {
    mocks.attachAIStudioImage.mockResolvedValue({
      id: "img-1",
      fileName: "ref.png",
      format: "png",
      width: 120,
    ...
    const response = await imagesPOST(
    ...
    expect(mocks.attachAIStudioImage.mock.calls[0][2].data).toEqual(
      Buffer.from([1, 2, 3]),
    );
  });
  ...
  it("rejects an oversized image before buffering it for the service", async () => {
    ...
    form.append(
      "file",
      new File([new Uint8Array(5 * 1024 * 1024 + 1)], "ref.png", {
        type: "image/png",
      }),
    );
    const response = await imagesPOST(
  ...
  it("maps validation failures to 422 with a localized detail code", async () => {
    mocks.attachAIStudioImage.mockRejectedValue(
      new AIStudioError("IMAGE_VALIDATION_ERROR", "Formato inválido.", {
        detailCode: "UNSUPPORTED_FORMAT",
      }),
    );
    const form = new FormData();
    form.append(
      "file",
      new File([new Uint8Array([1])], "ref.svg", { type: "image/svg+xml" }),
    );
    const response = await imagesPOST(
      new NextRequest("http://x/api/ai/studio/images", {
        method: "POST",
  ...
  it("lists the current temporary references without exposing bytes", async () => {
    mocks.listAIStudioImages.mockReturnValue([
      {
        id: "img-1",
        fileName: "ref.png",
        format: "png",
  ...
  it("releases the listed image ids through the discard seam", async () => {
    ...
    expect(response?.status).toBe(200);
    expect(mocks.discardAIStudioImage).toHaveBeenCalledWith(
      "tenant-1",
      "user-1",
      "img-1",
    );
  ...
  it("clears all temporary images when leaving the session", async () => {
    const response = await imagesDELETE(
      new NextRequest("http://x/api/ai/studio/images", { method: "DELETE" }),
    );
    expect(response?.status).toBe(200);
    expect(mocks.clearAIStudioImages).toHaveBeenCalledWith(
      "tenant-1",
      "user-1",
    );
  ...
  it("discards the ephemeral session snapshot at the API boundary", async () => {
    const response = await sessionDELETE(
      new NextRequest("http://x/api/ai/studio/session", {
        method: "DELETE",
    ...
    expect(response?.status).toBe(200);
    expect(mocks.discardAIStudioSession).toHaveBeenCalledWith(
      "tenant-1",
      "user-1",
      "session-1",
    );
