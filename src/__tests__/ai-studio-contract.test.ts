...
describe("AI Studio text contract", () => {
  it("builds a versioned prompt snapshot with the official variable catalog", () => {
    ...
    expect(snapshot.variableCatalog.map((item) => item.name)).toContain(
      "itens",
    );
    expect(
      snapshot.variableCatalog.find((item) => item.name === "itens")?.semantics,
    ).toContain("renderer");
  ...
  it("accepts the structured candidate only when custom variables are declared", () => {
    const candidate = validateCandidateContract({
      ...
      customVariables: [
        { name: "prazo.entrega", description: "Prazo combinado." },
      ],
      sessionSummary: {
        focus: "Briefing convertido em candidato.",
        decisions: [],
        pending: [],
        variables: [],
      },
  ...
  it("parses fenced provider JSON and reports variable changes", () => {
    ...
    expect(
      compareVariables("<p>{{cliente.nome}}</p>", "<p>{{proposta.titulo}}</p>"),
    ).toEqual({
  ...
  it("repairs a provider response that double-escapes HTML attribute quotes", () => {
    ...
    expect(parseStructuredOutput(validJsonWithOverescapedHtml)).toEqual(
      expected,
    );
  });
  ...
  it("reports removals and additions across the base and the candidate", () => {
    ...
    const after =
      "<p>{{cliente.nome}}</p><p>{{prazo.entrega}}</p><p>{{valor.frete}}</p>";
  ...
  it("scopes the session summary as informational context that cannot change HTML", () => {
    const { systemPrompt, userPrompt } = buildStudioPrompts({
      ...
      sessionSummary: {
        focus: "Decisões da sessão: paleta azul.",
    ...
    expect(systemPrompt).toContain(
      "focus, decisions, pending and variables arrays",
    );
  ...
  it("requires a real summary object and keeps long transcript messages provider-safe", () => {
    ...
    expect(compactSessionMessage(longMessage).length).toBeLessThanOrEqual(
      4_000,
    );
  });
  ...
  it("bounds accumulated decisions when two individually valid summaries are merged", () => {
    const previous = validateSessionSummary({
      ...
      decisions: Array.from(
        { length: 6 },
        (_, index) => `previous-${index}-${"x".repeat(390)}`,
      ),
      pending: [],
      variables: [],
    });
    const next = validateSessionSummary({
      ...
      decisions: Array.from(
        { length: 6 },
        (_, index) => `next-${index}-${"y".repeat(390)}`,
      ),
      pending: [],
      variables: [],
    });
