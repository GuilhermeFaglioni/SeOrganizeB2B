...
const migration = [
  readFileSync(resolve(__dirname, "../migrations/20260811152611_add_plans/migration.sql"), "utf-8"),
  readFileSync(resolve(__dirname, "../migrations/20260826110000_monthly_ai_studio_credits/migration.sql"), "utf-8"),
].join("\n");
...
describe("plans schema contract", () => {
  it("defines the Plan model with subscription fields", () => {
    ...
    expect(plan).toContain('monthlyAiStudioCredits Int?    @map("monthly_ai_studio_credits")');
  ...
  it("constrains monthly AI Studio credits to non-negative values", () => {
    expect(migration).toContain('ADD COLUMN "monthly_ai_studio_credits" INTEGER');
    expect(migration).toContain('"monthly_ai_studio_credits" >= 0');
  });
});
